...
@Component({
  ...
  templateUrl: './dinner-modal.component.html',
  styleUrls: ['./dinner-modal.component.css']
})
export class DinnerModalComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DinnerModalComponent>,
  ...
  ngOnInit(): void {
    ...
    this.dinnerForm = this.fb.group({
      ...
      dinnerTime: [
        existingData?.dinnerTime ? this.formatTimeForInput(existingData.dinnerTime) : '',
        Validators.required
      ]
    });
  }
  ...
  onSave(): void {
    if (this.dinnerForm.valid) {
      ...
      formValue.dinnerTime = `${yyyy}-${mo}-${dd}T${hr}:${min}:${sec}`;
