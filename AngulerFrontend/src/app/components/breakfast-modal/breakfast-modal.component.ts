...
@Component({
  ...
  templateUrl: './breakfast-modal.component.html',
  styleUrls: ['./breakfast-modal.component.css']
})
export class BreakfastModalComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<BreakfastModalComponent>,
  ...
  ngOnInit(): void {
    ...
    this.breakfastForm = this.fb.group({
      ...
      breakfastTime: [
        existingData?.breakfastTime ? this.formatTimeForInput(existingData.breakfastTime) : '',
        Validators.required
      ]
    });
  }
  ...
  onSave(): void {
    if (this.breakfastForm.valid) {
      ...
      formValue.breakfastTime = `${yyyy}-${mo}-${dd}T${hr}:${min}:${sec}`;
